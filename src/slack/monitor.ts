...
export async function monitorSlackProvider(opts: MonitorSlackOpts = {}) {
  ...
  app.event("channel_topic", async ({ event }) => {
    try {
      const payload = event as SlackChannelTopicEvent;
      const channelId = payload.channel;
      const channelInfo = channelId ? await resolveChannelName(channelId) : {};
      if (
        !isChannelAllowed({
      ...
      const topic = payload.topic?.value?.trim();
      enqueueSystemEvent(
        topic
          ? `Slack topic set in ${label}: ${topic}`
          : `Slack topic cleared in ${label}.`,
        {
          contextKey: `slack:channel:topic:${channelId ?? "unknown"}`,
        },
      );
    } catch (err) {
      runtime.error?.(danger(`slack topic handler failed: ${String(err)}`));
    }
  });

  app.event("channel_purpose", async ({ event }) => {
    try {
      const payload = event as SlackChannelPurposeEvent;
      const channelId = payload.channel;
      const channelInfo = channelId ? await resolveChannelName(channelId) : {};
      if (
        !isChannelAllowed({
      ...
      const purpose = payload.purpose?.value?.trim();
      enqueueSystemEvent(
        purpose
          ? `Slack purpose set in ${label}: ${purpose}`
          : `Slack purpose cleared in ${label}.`,
        {
          contextKey: `slack:channel:purpose:${channelId ?? "unknown"}`,
        },
      );
    } catch (err) {
      runtime.error?.(danger(`slack purpose handler failed: ${String(err)}`));
    }
  });
