    ...
    case "slack": {
      ...
      return html`
        <div class="card">
          ...
          <div class="card-sub">Socket mode connection and probe status.</div>
          ...
          <div class="form-grid" style="margin-top: 16px;">
            ...
            <label class="field">
              ...
              <input
                ...
                placeholder="U123, U456 or *"
              />
            </label>
            <label class="field">
              <span>Enable group DMs</span>
              <select
            ...
            <label class="field">
              <span>Reply to mode</span>
              <select
                .value=${props.slackForm.replyToMode}
                @change=${(e: Event) =>
                  props.onSlackChange({
                    replyToMode: (e.target as HTMLSelectElement).value as SlackForm["replyToMode"],
                  })}
              >
                <option value="off">Off</option>
                <option value="first">First reply only</option>
                <option value="all">Every reply</option>
              </select>
            </label>
            <label class="field">
              <span>Reaction notifications</span>
              <select
                ...
                @change=${(e: Event) =>
                  props.onSlackChange({
                    reactionNotifications: (e.target as HTMLSelectElement)
                      .value as SlackForm["reactionNotifications"],
                  })}
              >
                <option value="own">Own messages only</option>
          ...
          <div class="card-sub" style="margin-top: 16px;">Slash commands</div>
          ...
          <div class="card-sub" style="margin-top: 16px;">Channel rules</div>
          ${props.slackForm.channels.length === 0
            ? html`<div class="muted" style="margin-top: 8px;">
                No channel rules yet. Add one to restrict or require mentions.
              </div>`
            : nothing}
            ...
            (channel, index) => html`
              <div class="form-grid" style="margin-top: 10px;">
                ...
                <button
                  class="btn"
                  @click=${() => {
                    const next = props.slackForm.channels.filter(
                      (_, idx) => idx !== index,
                    );
                    props.onSlackChange({ channels: next });
                  }}
          ...
          ${props.slackTokenLocked || props.slackAppTokenLocked
            ? html`<div class="callout" style="margin-top: 12px;">
                Slack tokens are set in the environment. Config edits will not override them.
              </div>`
            : nothing}
